...
每次 PaperJury workflow 开始时，插件会对 GitHub 稳定 release tag 做一次软更新检查。
如果有新版，它会给出 latest channel 和 pinned release 的安装命令；如果 GitHub 不可达，
它会静默继续。设置 `PAPERJURY_DISABLE_UPDATE_CHECK=1` 可以关闭提醒。重新安装或切换
release channel 后，建议新开一个 Codex thread，让新版 skill 内容被重新加载。
