...
(or under a project-scoped skill folder when your host supports one). The skill is
discovered through `SKILL.md` and appears as `paperjury`. `node` is required for
deterministic checks; a LaTeX toolchain is optional because compile checks degrade
...
- **Codex plugin packaging**, if Codex exposes a stable marketplace/install route for community skills.
